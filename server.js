...
const EDGE_URL = process.env.SUPABASE_FUNCTION_URL || 'https://usqsewecifxrxixprqxa.supabase.co/functions/v1/artemis-api';
...
function isAdmin(req){const sid=cookies(req)['artemis.sid'];return !!sid && (sessions.get(sid)||0)>Date.now()}
...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/admin/login'){
    ...
    const sid=crypto.randomBytes(32).toString('hex');sessions.set(sid,Date.now()+12*60*60*1000);headers(res,200,{'Content-Type':'application/json; charset=utf-8','Set-Cookie':`artemis.sid=${sid}; HttpOnly; SameSite=Lax; Max-Age=43200; Path=/${process.env.NODE_ENV==='production'?'; Secure':''}`});return res.end(JSON.stringify({ok:true}));
  }
  if(req.method==='POST'&&url.pathname==='/api/admin/logout'){const sid=cookies(req)['artemis.sid'];if(sid)sessions.delete(sid);headers(res,200,{'Content-Type':'application/json; charset=utf-8','Set-Cookie':'artemis.sid=; HttpOnly; SameSite=Lax; Max-Age=0; Path=/'});return res.end(JSON.stringify({ok:true}));}
