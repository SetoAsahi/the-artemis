...
function render(){
  ...
  const a=SITE.access||{};q('#accessInfo').innerHTML=[a.postal,a.address,a.route,`TEL. ${a.tel||''}`,a.hours].filter(Boolean).map(x=>`<p>${esc(x)}</p>`).join('');q('#mapLink').href=a.mapUrl||'https://maps.google.com';
