...
function renderCastOps(){
  ...
  todayRoot.innerHTML=site.cast.length?site.cast.map(c=>`<div class="today-row"><input type="checkbox" data-today-id="${esc(c.id)}" ${todaySet.has(String(c.id))?'checked':''}><span><strong>${esc(c.name||'名称未設定')}</strong><small style="display:block;margin:7px 0 4px;color:#897d6c">勤務時間</small><input type="text" data-shift-id="${esc(c.id)}" value="${esc(c.shift||'')}" placeholder="例：21:00 - LAST" style="width:100%;height:auto;padding:8px 10px;background:#070605;border:1px solid rgba(205,167,95,.28);color:#fff;accent-color:auto"></span></div>`).join(''):'<p>在籍キャストを先に登録してください。</p>';
