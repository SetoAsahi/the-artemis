...
const titles={dashboard:'ダッシュボード',content:'基本情報',cast:'在籍キャスト',castops:'ランキング・出勤',system:'料金',news:'ニュース',reservations:'予約',recruits:'求人応募'};
...
function renderCast(){
  ...
  root.innerHTML=(site.cast||[]).map((c,i)=>`<article class="edit-card" data-cast-index="${i}"><div class="thumb">${c.image?`<img src="${esc(c.image)}">`:`CAST ${esc(c.rank)}`}</div><div class="fields"><input data-k="rank" value="${esc(c.rank)}" placeholder="表示番号"><input data-k="name" value="${esc(c.name)}" placeholder="名前"><input data-k="roman" value="${esc(c.roman)}" placeholder="英字名"><input data-k="shift" value="${esc(c.shift)}" placeholder="出勤時間"><div class="card-actions"><button type="button" class="upload" data-photo>画像変更</button><button type="button" class="danger" data-delete>削除</button></div></div></article>`).join('');
...
function renderCastOps(){
  ...
  rankingRoot.innerHTML=[0,1,2].map(i=>`<div class="rank-select-card"><strong>TOP ${i+1}</strong><label>キャスト<select data-ranking-index="${i}">${options(site.ranking[i])}</select></label></div>`).join('');
  ...
  todayRoot.innerHTML=site.cast.length?site.cast.map(c=>`<label class="today-row"><input type="checkbox" data-today-id="${esc(c.id)}" ${todaySet.has(String(c.id))?'checked':''}><span><strong>${esc(c.name||'名称未設定')}</strong><small>${esc(c.shift||'出勤時間未設定')}</small></span></label>`).join(''):'<p>在籍キャストを先に登録してください。</p>';
...
$('#addCast').addEventListener('click',()=>{const c={id:uid('cast'),rank:String(site.cast.length+1).padStart(2,'0'),name:'新しいキャスト',roman:'New Cast',shift:'21:00 - LAST',image:''};site.cast.push(c);renderCast();renderCastOps();updateStats();markDirty()});
...
$('#saveAllButton').addEventListener('click',async()=>{try{normalizeSite();const out=await api('/api/admin/site',{method:'PUT',body:JSON.stringify({site})});site=out.site;DATA.site=site;normalizeSite();$('#saveState').textContent='保存済み';toast('公開サイトに保存しました')}catch(err){toast(err.message)}});
